...
function compile(_ref) {
  ...
  Object.keys(contracts).forEach(function (contractName) {
    ...
    var pagesFolderPath = output.substring(0, output.lastIndexOf("/"));
    var doxityFolderPath = pagesFolderPath.substring(0, pagesFolderPath.lastIndexOf("/"));
    var doxity_simpletonFolderPath = doxityFolderPath.substring(0, doxityFolderPath.lastIndexOf("/"));
    ...
    var contract_type = 'generic contract';
    var substring = 'library ' + contractName + ' {';
    let contractFile = _fs.readFileSync(doxity_simpletonFolderPath + '/contracts/' + contractName + '.sol');
    if (typeof contractFile != 'undefined')
      if (contractFile.includes(substring))
        contract_type = 'library';

    contract_type = ' [' + contract_type + ']';


    // Read JSON contract addresses list 
    let jsonObj = require(doxity_simpletonFolderPath + '/contractAddressesList.json');
    if (typeof jsonObj != 'undefined')
    {
      if (typeof jsonObj[contractName] != 'undefined') 
      {
        var address = jsonObj[contractName]['address']
        var networkID = jsonObj[contractName]['networkID']
        var chainID = jsonObj[contractName]['chainID']
        furtherDetails = ' - ' + address + ' [Net ' + networkID + ', Chain ' + chainID + ']';
      }
    }
    ...
    var data = {
      author: author,
      title: title + contract_type,
      fileName: fileName.replace(process.env.PWD, '') + furtherDetails,
      // address,
      name: contractName,
    ...
    // Write Doxity metadata to doxity-metadata-files directory
    _fs2.default.writeFileSync(doxity_simpletonFolderPath + '/doxity-metadata-files/' + contractName + '.json', JSON.stringify(contract) + '\n');
