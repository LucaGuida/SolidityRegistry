...
function compile(_ref) {
  ...
  Object.keys(contracts).forEach(function (contractName) {
    ...
    request.open('GET', doxity_simpletonFolderPath + '/contractAddressesList.json', false);  // `false` makes the request synchronous
    request.send(null);

    if (request.status === 200) { // TODO: ancora non viene eseguita questa riga!!!
          process.stdout.write('>>>118');
      jsonObj = JSON.parse(request.responseText);
      if (typeof jsonObj[contractName] != 'undefined')
        contractAddress = ' - ' + jsonObj[contractName];
    }
    ...
    var data = {
      author: author,
      title: title,
      fileName: fileName.replace(process.env.PWD, '') + contractAddress,
      // address,
      name: contractName,
